...
export function TabsList(props: React.ComponentProps<typeof Tabs.List>) {
  return <Tabs.List {...props} className={cn("inline-flex h-11 items-center gap-1 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 p-1 text-gray-700 border border-blue-100", props.className)} />;
}
export const TabsTrigger = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Tabs.Trigger>>(function TabsTrigger({ className, ...p }, ref) {
  return <Tabs.Trigger ref={ref} {...p} className={cn("px-4 py-2 text-sm rounded-lg transition-colors data-[state=active]:bg-white data-[state=active]:text-gray-900 data-[state=active]:shadow data-[state=inactive]:text-gray-600 hover:bg-white/70", className)} />;
});
...
export function Button({ className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={cn("inline-flex items-center justify-center rounded-lg bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-4 py-2 text-sm shadow-sm hover:from-indigo-500 hover:to-blue-500 active:scale-[.99] disabled:opacity-50 disabled:grayscale", className)} />;
}
